...
export default function CategoryFormScreen() {
    ...
    const handlePickImage = async () => {
        ...
        if (permissionResult.granted === false) {
            showAlert({ title: 'Permiso requerido', message: 'Es necesario el permiso para acceder a la galería.', icon: 'image-off', iconColor: '#f59e0b' });
            return;
        }
    ...
    const handleSave = () => {
        if (!name.trim()) {
            showAlert({ title: 'Error', message: 'El nombre es obligatorio', icon: 'alert-circle', iconColor: '#ef4444' });
            return;
        }
    ...
    return (
        <>
            <CustomHeader title={isEditing ? 'Editar Categoría' : 'Nueva Categoría'} />
